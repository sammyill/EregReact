...
function initLessonRoutes(app) {
    ...
    //COMPLETATE,IT WORKS
    //getting all the lessond of a specific month
    app.get('/getalllessons/:idcourse/:month/:year', authenticateToken, async (req, res) => {
        ...
        try {
            ...
            const [allMonthLesson] = await con.execute(`select l.id as idlesson,m.name as modulename,l.title as lessontitle,u.firstname as proffirst,u.lastname as proflast,l.begindate,l.enddate from lessons l 
            ...
            const [firstid] = await con.execute(`select l.id from lessons l 
                                            inner join modules m on l.id_modules =m.id
                                            inner join courses c on c.id=m.id_course 
                                            where c.id= ?
            ...
            const [lastid] = await con.execute(`select l.id from lessons l 
                                        inner join modules m on l.id_modules =m.id
                                        inner join courses c on c.id=m.id_course 
                                        where c.id= ? 
                                        order by l.begindate desc
                                        limit 1`,[idcourse]);
            let previous=(firstid[0]["id"]<allMonthLesson[0]["idlesson"]) ? true:false;
            let next=(lastid[0]["id"]>allMonthLesson[allMonthLesson.length-1]["idlesson"]) ? true:false;
            const data={
    ...
    //DOVREBBE FUNZIONARE,DA TESTARE
    ...
    app.post('/createlesson', jsonParser, authenticateToken, async (req, res) => {
        ...
        try {
            ...
            if (validation[0].length < 1) {
                //lesson creation
                const [data] = await con.execute(`insert into lessons (title,description,begindate,enddate,id_modules) values (?,?,?,?,?)`, 
                                                                [rqbody.title, rqbody.description, rqbody.begindate, rqbody.enddate, rqbody.idmodule]);
                res.json(data);
            } else {
    ...
    app.patch('/updatelesson/:idcourse/:idlesson', jsonParser, authenticateToken, async (req, res) => {
        ...
        try {
            ...
            //update lesson
            const data = await con.execute(`update lessons set begindate = ?, enddate = ?, description = ?, title = ? where id = ?`,
                                                 [rqbody.begindate, rqbody.enddate, rqbody.description, rqbody.title, patchid]);
            res.json(data);
